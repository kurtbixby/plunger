...
function GameStatusWidget(props) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: [queryKeyConstants.homePageLists],
      });
    },
  });
  ...
  function changeNumericTextField(text) {
    ...
    if (decimalIdx !== -1) {
      ...
      filteredText =
        filteredText.substring(0, decimalIdx) +
        "." +
        filteredText.substring(decimalIdx);
    }
    if (!isNaN(filteredText)) {
      ...
      return filteredText.substring(
        0,
        Math.min(filteredText.length, decimalIdx + 1 + 2),
      );
    } else {
      return text;
    }
  ...
  return (
    <div className="flex flex-row bg-fuchsia-400 currentStatus">
      <SmallRowElement
        label="Date Started"
        body={dateFormat(status.dateStarted)}
      />
      <form className="flex grow" onSubmit={saveChanges}>
        <div className="flex grow flex-col content-start items-center">
          <label className="text-center" htmlFor="currentHours">
            Current Status
          </label>
          <WrappedSelect
            name="status"
            styles="text-center"
            contents={UIConstants.PlayStates.list}
            value={playStatus}
            onSelect={(e) => setPlayStatus(e.target.value)}
          />
        </div>
        <div className="flex grow flex-col content-start items-center">
          <label className="text-center" htmlFor="currentHours">
