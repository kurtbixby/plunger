...
function SmallCollectionEntry(props) {
    const { game: { collectionEntries, game } } = props;
        ...
        <div className="collectionEntry">
            {collectionEntries.map((entry) => (
                <div className="m-1 bg-amber-200 flex content-around" key={entry.id}>
                    ...
                    <SmallRowElement label={"Physicality"} body={Enums.Physicality[entry.physicality]}/>
                    <SmallRowElement label={"Date Acquired"} body={dateFormat(entry.timeAcquired)}/>
                    {entry.purchasePrice && <SmallRowElement label={"Purchase Price"} body={formatCurrency(entry.purchasePrice)} />}
                    <SmallRowElement label={"Date Added"} body={dateFormat(entry.timeAdded)}/>
                </div>
            ))}
