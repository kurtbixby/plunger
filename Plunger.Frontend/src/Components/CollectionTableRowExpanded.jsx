                ...
                <div className="basis-2/3">
                    <div className="flex">
                        <div className="grow basis-1/3 flex flex-col justify-evenly">
                            <TableSelect isEditable={isEditable} name="Platform" textValue={collectionEntry.game.platforms.find(p => p.id === formState.platform).name} value={formState.platform} options={collectionEntry.game.platforms} onSelect={e => updateFormField("platform", +(e.target.value))}/>
                            <TableSelect isEditable={isEditable} name="Region" textValue={collectionEntry.region.name} value={formState.region} options={UIConstants.Regions.list} onSelect={e => updateFormField("region", +(e.target.value))}/>
                            <TableSelect isEditable={isEditable} name="Physicality" textValue={Enums.Physicality[formState.physicality]} value={formState.physicality} options={UIConstants.Physicalities.list} onSelect={e => updateFormField("physicality", +(e.target.value))}/>
                        </div>
                        <div className="grow basis-1/3 flex flex-col justify-evenly">
                            <TableCurrency isEditable={isEditable} name="Price" value={formState.purchasePrice} onChange={e => updateFormField("purchasePrice", e.target.value)}/>
                            <TableDate isEditable={isEditable} name="Date Acquired" value={formState.timeAcquired} onChange={e => updateFormField("timeAcquired", e.target.value)}/>
                            <TableDate isEditable={false} name="Date Added" value={formState.timeAdded}/>
                        </div>
                        {hasStatus && <div className="grow basis-1/3 flex flex-col justify-evenly">
                            <TableCheckbox isEditable={isEditable} name="Completed" value={formState.completed} onChange={e => updateFormField("completed", e.target.checked)}/>
                            <TableSelect isEditable={isEditable} name="Current Status" textValue={Enums.PlayStates[formState.playState]} value={formState.playState} options={UIConstants.PlayStates.list} onSelect={e => updateFormField("playState", +(e.target.value))}/>
                            <TableTime isEditable={isEditable} name="Current Hours" value={formState.timePlayed} onChange={e => updateFormField("timePlayed", e.target.value)}/>
                        </div>}
                    </div>
                    {isEditable && <div>
