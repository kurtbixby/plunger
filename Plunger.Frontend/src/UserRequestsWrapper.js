...
async function makePatchRequest(url, payload, extHeaders = {}, withAuth = true){
...
async function makeRequest(method, url, payload, extHeaders = {}, withAuth = true) {
    // Can handle refreshing the token is desired
    if (withAuth) {
        extHeaders["Authorization"] = `Bearer ${accessToken}`;
    }
    return await makeBaseRequest(method, combinedBaseUrl + url, payload, extHeaders);
}

export default { updateToken, clearToken, makeGetRequest, makePostRequest, makePatchRequest, makeDeleteRequest };